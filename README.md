...
ローカルで4・8・16ページマンガのネーム生成、ページレンダリング、PNG/CBZ出力を行う制作システムです。既定の`auto`設定ではComfyUIを生成時に再検出し、利用できない場合はstubへ退避します。
...
## SQLite migrationとバックアップ

起動時にSQLiteのschema migrationを自動適用します。既存DBを移行する前に、必要に応じてバックアップしてください。

ファイルコピーでバックアップする場合は、**バックエンド本体とUvicornのreload子プロセスを含む、DBを使用する全プロセスを停止してから**実行してください。稼働中に`.db`、`-wal`、`-shm`を順番にコピーしても、同じ時点の整合したバックアップにはなりません。

停止後のコピー例:

```powershell
Copy-Item data\local-doujin-studio.db data\local-doujin-studio.db.bak
Copy-Item data\local-doujin-studio.db-wal data\local-doujin-studio.db-wal.bak -ErrorAction SilentlyContinue
Copy-Item data\local-doujin-studio.db-shm data\local-doujin-studio.db-shm.bak -ErrorAction SilentlyContinue
```

アプリ本体は自動バックアップを作成しません。migrationに失敗した場合や、このアプリより新しいschema versionを検出した場合は、日本語エラーを出して起動を停止します。
...
```powershell
$env:LLM_PROVIDER = "auto"                # auto | stub | openai_compatible
...
`auto`ではLM Studioの`/models`を生成直前にも確認し、`LLM_MODEL`が空ならロード済みモデルの先頭を使います。モデルを後からロードした場合は、ストーリー生成画面の`ロード済みモデルを再検出`を押せば状態表示も更新されます。ロード済みモデルがない場合はstubへ退避し、画面に警告を表示します。`GET /api/llm/status`でも接続状態とモデル一覧を確認できます。
...
ブラウザ画面を開かずheadlessで起動する場合は、ComfyUIの配置先を指定します。プロセスを起動したままにしておけば、アプリ側は各生成時に自動検出するため再起動不要です。

```powershell
.\scripts\start-comfy-headless.ps1 -ComfyPath "C:\tools\ComfyUI"
```
...
既定の`IMAGE_BACKEND=auto`では、生成時点でComfyUI接続とworkflowを確認します。接続不能時はstub画像へフォールバックし、画面に警告を表示します。workflow不正などの設定エラーは診断できるようエラーとして扱います。NoobAI、Illustrious、各種LoRAなどのモデル名や配置はユーザーのComfyUI環境に依存します。
...
- `ページ編集`でコマを選択すると、長方形・左右傾斜・台形を切り替えられます
