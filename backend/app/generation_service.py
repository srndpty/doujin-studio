...
# --- 生成ワーカー・キャンセル・ComfyUI停止・panel終端化 ---
#
# docs/refactoring-plan.md: GenerationServiceの責務として、ジョブ実行・候補保存・キャンセル・
# ComfyUI停止・panel終端化を集約する。JobManagerはジョブ状態管理と永続化に限定する。
# これらはapp.state経由でmutation/rendering/job_managerを使うため、HTTP境界を持たない
# module-level関数として置く（HTTPExceptionには依存しない）。
    ...
    try:
        ...
        for candidate_index in range(job.candidate_count):
            ...
            if job.status == "cancelled":
                app.state.rendering.cleanup_published_assets(job.project_id, candidate_assets)
                mark_panel_job_stopped(app, job, "生成をキャンセルしました")
                return
            if result.asset_path is None:
